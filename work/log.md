...
- [change] Wide-screen gutter rule was being overridden by the base .pad rule (equal specificity, later source order), so procurement, footer, and trust-stat content spread full width; moved the min-width:1441px override after the base rule. All section content now holds the 1440 grid on wide screens.
- [progress] Added subtle motion and interaction layer, guided by the freshtechbro/claudedesignskills interaction-pattern catalog and Leonxlnx/taste-skill (its trust-first dial: motion 2-3, redesign-preserve). Hover states on buttons/cards/links/nav/logos, animated FAQ chevrons, gentle staggered scroll reveals, one-time stat count-up. Pure CSS + IntersectionObserver, no libraries; page rests fully visible without JS; all motion disabled under prefers-reduced-motion. Design unchanged.
- [change] About ICC-IMS button no longer stretches full column width (why-left column was defaulting to stretch alignment; now flex-start). Mobile full-width CTA behavior unchanged.
